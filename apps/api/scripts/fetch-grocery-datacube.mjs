...
const TARGET_URL =
  'https://datacube.statistics.sk/#!/view/sk/vbd_sk_win2/sp1828ms/v_sp1828ms_00_00_00_sk';
const USER_DATA_DIR = '/tmp/codex-datacube-groceries';
...
async function waitForCubeFrame(send) {
  for (let attempt = 0; attempt < 45; attempt += 1) {
    const result = await send('Runtime.evaluate', {
      expression: `(() => {
        const frame =
          document.querySelector('iframe[title*="sp1828ms"]') ||
          document.querySelector('iframe[src*="Cube=sp1828ms"]');
  ...
  throw new Error('The DATAcube grocery iframe did not become ready in time.');
}
...
async function fetchSnapshots(send) {
  const result = await send('Runtime.evaluate', {
    expression: `(() => {
      const frame =
        ...
        document.querySelector('iframe[src*="Cube=sp1828ms"]');
      if (!frame) {
        throw new Error('The grocery DATAcube iframe was not found.');
      }

      const src = frame.getAttribute('src') || '';
      const token = new URL(src, location.origin).hash.match(/SessionToken=([^&]+)/)?.[1];
      if (!token) {
        throw new Error('The grocery DATAcube session token was not available.');
      }
      ...
      const mergePageIntoSnapshots = (data, snapshots, headerState) => {
        ...
        const months = (columnHeaderRows[1]?.cells || []).map((cell) => cell?.value ?? '');
        ...
        const itemLabels =
          (rowHeaderColumns[0]?.cells || []).map((cell) => cell?.value ?? '') || [];
        const units =
          (rowHeaderColumns[1]?.cells || []).map((cell) => cell?.value ?? '') || [];

        if (itemLabels.length > 0) headerState.itemLabels = itemLabels;
        if (units.length > 0) headerState.units = units;

        const activeItemLabels = headerState.itemLabels || [];
        const activeUnits = headerState.units || [];
        ...
        for (let rowIndex = 0; rowIndex < dataRows.length; rowIndex += 1) {
          const itemLabel = activeItemLabels[rowIndex] || '';
          const unit = activeUnits[rowIndex] || '';
          if (!itemLabel || !unit) continue;

          const itemCode = itemLabel + '__' + unit;
          ...
          for (let colIndex = 0; colIndex < rowCells.length; colIndex += 1) {
            ...
            if (!existing.some((item) => item.itemCode === itemCode)) {
              existing.push({
                itemCode,
                itemLabel,
                unit,
            ...
            tm1Service.newCubeViewWidget(
              ...
              true,
              {
                callback: (widget) => {
                        ...
                        errorHandler: (message, error) =>
                          pageResolve({
                            ...
                            error:
                              message ||
                              error?.message ||
                              'The DATAcube grocery view did not return data.',
                          }),
                        });
                    });
                        ...
                        errorHandler: (message, error) =>
                          pageResolve({
                            ...
                            error:
                              message ||
                              error?.message ||
                              'The DATAcube grocery view could not navigate to the next page.',
                          }),
                      });
                    });
                  ...
                  (async () => {
                    const snapshots = {};
                    const headerState = { itemLabels: null, units: null };
                    ...
                    resolve({
                      error: error?.message || 'The DATAcube grocery view could not be processed.',
                    });
                  });
                },
                errorHandler: (message, error) =>
                  resolve({
                    error:
                      message ||
                      error?.message ||
                      'The DATAcube grocery widget could not be created.',
                  }),
              },
            );
          },
  ...
  if (!value) {
    throw new Error('The DATAcube grocery fetch returned no result.');
  }
  if (value.error) {
